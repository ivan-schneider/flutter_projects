...
BEGIN
	...
	RETURN QUERY
		SELECT lista.dat_inicio, lista.dat_fim, lista.cod_tipo_evento, 
		       lista.dsc_evento, lista.sgl_evento, lista.flg_na_escala
		...
		AS  lista(dat_inicio timestamp without time zone, dat_fim timestamp without time zone, cod_tipo_evento integer, 
				  dsc_evento text, sgl_evento text, flg_na_escala boolean);
    RETURN;
END;
$BODY$;
