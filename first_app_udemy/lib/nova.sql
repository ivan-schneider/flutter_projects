...
CREATE OR REPLACE FUNCTION graphql_brado.fn_brado_lista_agenda_militar(
	p_cod_pessoa_servidor bigint,
	p_dat_ini_periodo timestamp without time zone,
	p_dat_fim_periodo timestamp without time zone)
    RETURNS SETOF graphql_brado.ret_brado_lista_agenda_militar 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	-- 1. ESCALA E SAIU DA ESCALA
		...
		ON TRUE
		WHERE cod_ala_servico IS NOT NULL
	...
	-- 3. FORA DA ESCALA
		SELECT 	6 AS cod_tipo_evento,  
				...
				NULL AS cod_ala_servico,
				NULL AS cod_grupo_ala,
				3 AS num_prio
		FROM json_to_recordset( v_alas )
		AS  alas(cod_pessoa_servidor bigint, dia date, cod_orgao integer, sgl_orgao character varying, cod_ala_servico_base integer, 
				 dat_referencia text, cod_grupo_ala integer, nom_grupo_ala text, 
	...
	RETURN QUERY
		SELECT lista.dat_inicio, lista.dat_fim, lista.cod_tipo_evento, lista.dsc_evento, lista.sgl_evento, 
			   lista.cod_ala_servico, lista.cod_grupo_ala, lista.flg_na_escala
		FROM jsonb_to_recordset( v_jsonb_lista )
		AS  lista(dat_inicio timestamp without time zone, dat_fim timestamp without time zone, cod_tipo_evento integer, 
				  dsc_evento text, sgl_evento text, cod_ala_servico integer, cod_grupo_ala integer, flg_na_escala boolean);
    RETURN;
END;
$BODY$;
